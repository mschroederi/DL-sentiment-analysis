...
class LSTMClassifierTrainer:
    def __init__(self, use_grid_search: bool = False, param_grid: dict = default_param_grid):
        ...
        self.dataset_train, self.dataset_validation, self.dataset_test = None, None, None
        self.vocab_size, self.padding_size = None, None


    def __proprocess_datasets(self):
        print("Starting preprocessing pipeline...")
        if self.dataset_train is None or self.dataset_validation is None or self.dataset_test is None:
    ...
    def __evaluate_on_test_data(self, model):
    ...
    def train(self, train_data_path: str = 'data/train.csv', test_data_path: str = 'data/test.csv', num_epochs: int = 50, patience: int = 5):
        ...
        self.__proprocess_datasets()

        if self.use_grid_search:
            grid_search = GridSearch(self.param_grid, self.__train_with_config)
            best_model = grid_search.run(num_epochs, patience)
            self.__evaluate_on_test_data(best_model)
        else:
            _, model = self.__train_with_config(best_param_config, num_epochs, patience)
            self.__evaluate_on_test_data(model)
